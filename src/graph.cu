...
void Graph::compileToFile(const std::string &filename, bool standalone, bool fused) {
    ...
    std::vector<Node*> &targetGraphs = fused ? fusedGraphs : sorted;
    if (nodeMemMap.size() < nodes.size()) {
        freeMem();
